...
impl Solver {
    ...
    fn new(contents: String) -> Solver {
        ...
        let mut fallen_blocks: HashMap<(usize), Vec<((usize, usize, usize), (usize, usize, usize))>> = HashMap::new();
        ...
        for line in contents.lines() {
            ...
            blocks.push(((start[0].parse().unwrap(), start[1].parse().unwrap(), start[2].parse().unwrap()), (end[0].parse().unwrap(), end[1].parse().unwrap(), end[2].parse().unwrap())));
        }
    ...
    fn fall_down(&mut self) {
        ...
        for block in self.blocks() {
            if block.0.2.min(block.1.2) == 1 {
                self.fallen_blocks.entry(1).or_insert(Vec::new()).push(block);
            } else {
                ...
                while level >= 1 {
                    if level == 1 {
                        ...
                        if let Some(fallen_block_level) = self.fallen_blocks.get(&(level - 1)) {
                            ...
                            if blocked {
                                self.fallen_blocks.entry(level).or_insert(Vec::new()).push(block);
                                break;
                            }
                        } else {
                            level -= 1;
